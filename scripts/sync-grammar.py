...
README ni del LSP, que está desactualizado. Se descarga el archivo de
`VelitRicardo/axon-lang`, se extraen las palabras clave reales y se regenera
`src/prism/axon.ts`.
...
Uso:  python scripts/sync-grammar.py [--ref master]
"""
...
RAW = "https://raw.githubusercontent.com/VelitRicardo/axon-lang/{ref}/axon-frontend/src/tokens.rs"
...
def fetch(ref: str) -> str:
    with urllib.request.urlopen(RAW.format(ref=ref), timeout=60) as r:
