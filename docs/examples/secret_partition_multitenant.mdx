...
description: "Multi-tenant secrets: one `tool` with `secret_partition:` injects each customer's own CRM OAuth credential at dispatch; the flow never sees the value."
---
