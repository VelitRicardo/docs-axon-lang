...
description: "OAuth token rotation: a cron `daemon` finds tenant credentials near expiry and `rotate` renews them via a tool, without the program reading a token."
---
