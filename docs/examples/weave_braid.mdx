...
description: "Ensemble reasoning with `weave`: braids several step outputs into one result under a composition style; later steps read `<target>`, not `<target>.output`."
---
